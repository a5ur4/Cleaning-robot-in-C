...
const int directions[4][2] = {{0, -1}, {0, 1}, {-1, 0}, {1, 0}}; // Direções: esquerda, direita, cima, baixo
    ...
    while (1) {
        ...
        while (x != targetX || y != targetY) {
            ...
            for (int i = 0; i < 4; i++) { // Itera sobre as direções para encontrar a direção com a menor distância para a sujeira
                ...
                if (isValidMove(newX, newY) && matrix[newX][newY] != 0 && matrix[newX][newY] != -2) { // Verifica se o movimento é válido e se não foi visitado
